...
export async function GET(req: any) {
  ...
  try {
    ...
    const [procesos]: any = await conexion.query(
      `SELECT Pe.id,texto, PE.escala FROM newProcesoEstudiante Pe INNER JOIN newProcesosEvaluacion PE ON PE.id = Pe.proceso INNER JOIN newBancoProcesos BP ON PE.relacionBanco = BP.id WHERE PE.periodo = ${periodo[0]?.per_id} AND Pe.estudiante = ${estudiante} AND Pe.cga = ${cga}`
    );
    ...
    return NextResponse.json(
      {
        body: "Error al consultar la información de procesos asociados al estudiante",
      },
      { status: 400 }
    );
  }
